...
> 這節寫**依賴什麼、缺什麼**，不寫長什麼樣。視覺由 Design System 決定，本節只負責讓它接得上。

**既有 Design System 狀態**（先拍板，決定 §5.9 交接內容）:
- [ ] **已有** — 沿用，下方填參照來源（Figma / Storybook / 程式庫連結）
- [ ] **沒有** — 本專案尚無 Design System。這是**前端開工的前置條件**，需於實作前產出（見 §5.9）

**來源**: {Figma / Storybook / 程式庫連結，或寫「本專案無既有 Design System，見 §5.9」}

**本 feature 需要、但既有 Design System 還沒有的**:
- {需求一句話 —— 寫「需要什麼」不寫「該長什麼樣」。
  例：「SLA 倒數需要三個級距的狀態表現（充裕 / 接近 / 已逾期）」，
  而不是「用綠 / 黃 / 紅」——選什麼顏色是設計的事}
...
**版面約束** — 每條的形式是「{什麼必須保持成立} —— 因為 {理由}」。寫不出「因為」的就不是約束，刪掉:
- {例：T-1 與 T-2 必須同時可見 —— 因為改 T-1 會即時改變 T-2 的可選項}
- {例：T-3 在手機上可收起 —— 因為它不是決策必需的資訊}

> 分辨方式：約束說「什麼必須保持成立」，版面說「東西放在哪」。
> 「捲動時必須保持可見」是約束 —— 貼頂、浮底、側欄都能滿足它。
> 「在頁面底部」「佔主要空間」「sticky」「z-index 最高」是版面與實作，歸設計。
> 一條約束如果只有一種排法能滿足，那它其實是版面，換個寫法或刪掉。
