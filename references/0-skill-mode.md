...
- **低風險決策不攔人**：有業界慣例或明顯建議值、未來改變成本低的決策（UI 配置、命名、寬鬆的 NFR 數值、可逆的結構選擇），直接採建議值，展示時一句帶過：「以下 N 項我直接採建議值（列清單），要改再說」。
...
- §5 Presentation Type 非 GUI 時，跳過所有 §5.4-5.6 相關項目（component / page / T-N）
...
⚠️ 陷阱：掃「殘留 §9 引用」時，`grep "§9"` 會誤命中 `FR-9`、`SF-9`、`AC-9.x` — 命中結果要逐筆判斷是否真的指向 rollout 文件，不能看到命中就當成有殘留。
